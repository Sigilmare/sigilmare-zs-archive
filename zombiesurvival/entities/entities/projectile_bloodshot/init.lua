	...
	if owner:IsValidLivingHuman() then
		for _, ent in pairs(ents.FindInSphere(hitpos, self.Radius * (owner.CloudRadius or 1))) do
			if ent and ent:IsValidLivingHuman() and WorldVisible(hitpos, ent:NearestPoint(hitpos)) then
				ent:SetArmor(math.min(ent:Armor() + 10 * ent.ArmorGainMul, ent:GetMaxArmor()))

				local strstatus = ent:GiveStatus("strengthdartboost", 8)
				strstatus.Applier = owner

				local defstatus = ent:GiveStatus("medrifledefboost", 12)
				defstatus.Applier = owner

				net.Start("zs_buffby")
					net.WriteEntity(owner)
					net.WriteString("Armor Protection Bomb")
				net.Send(ent)
			end
		end
	end
