...
function ENT:DrawTranslucent()
	...
	if drawinfo then
		...
		if displayowner == "" then
			...
			if deployer:IsValid() then
				...
				if deployer:Team() == TEAM_HUMAN and deployer:Alive() then
					local lvl = deployer:GetZSRemortLevel()
					
					hcolor = Color(128, 222, 234, 240)
					expert = lvl > 0
				else
		...
		cam.Start3D2D(nearest, ang, 0.1)
			...
			if validfriend then
				draw.SimpleTextOutlined("ZS Friend", "ZSHUDFontSmallerNS", 0, displayowner and 50 or 25, Color(255, 100, 100, 240 * vis), TEXT_ALIGN_CENTER, nil, 1, Color(0, 0, 0, 255 * vis))
			elseif expert then
				draw.SimpleTextOutlined("Locked to R.Level "..deployer:GetZSRemortLevel().."+", "ZSHUDFontSmallerNS", 0, displayowner and 50 or 25, Color(hcolor.r, hcolor.g, hcolor.b, 240 * vis), TEXT_ALIGN_CENTER, nil, 1, Color(0, 0, 0, 255 * vis))
			end
			...
			if self:GetMaxRepairs() > 0 or self:GetMaxNailHealth() > 0 then
				...
				draw.SimpleTextOutlined("Health: "..string.CommaSeparate(math.floor(nhp)) .. " / " .. string.CommaSeparate(math.floor(self:GetMaxNailHealth())), "ZSHUDFontSmallerNS", 0, -25, colNail, TEXT_ALIGN_CENTER, nil, 1, Color(0, 0, 0, 255 * vis))
				draw.SimpleTextOutlined("Repairs: "..string.CommaSeparate(math.floor(repairs)) .. " / " .. string.CommaSeparate(math.floor(mrps)), "ZSHUDFontSmallerNS", 0, 0, math.floor(repairs) == 0 and Color(255, 0, 0, 240 * vis) or Color(100, 170, 215, 240 * vis), TEXT_ALIGN_CENTER, nil, 1, Color(0, 0, 0, 255 * vis))
				local colowner = redname and colDead or colText
				colowner.a = 125 * vis
				if displayowner then
					draw.SimpleTextOutlined("Owner: "..displayowner, "ZSHUDFontSmallerNS", 0, 25, colowner, TEXT_ALIGN_CENTER, nil, 1, Color(0, 0, 0, 255 * vis))
				end
			end
		cam.End3D2D()
