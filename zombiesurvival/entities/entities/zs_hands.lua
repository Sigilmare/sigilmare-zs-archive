...
function ENT:DoSetup( ply, armsmdl )
	...
	if ZS.Playermodels[ply:SteamID()] and ZS.Playermodels[ply:SteamID()].ArmsModel and not ply.RisenAsHero then
		self:SetModel(ZS.Playermodels[ply:SteamID()].ArmsModel)
	elseif armsmdl then
		self:SetModel(armsmdl)
	else
