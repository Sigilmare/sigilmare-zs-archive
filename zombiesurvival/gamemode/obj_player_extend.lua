...
function meta:SetSpeed(speed)
	...
	local runspeed = self:Armor() > 0 and self:IsSkillActive(SKILL_CARDIOTONIC) and speed + 40 or speed
...
function meta:ResetSpeed(noset, health)
	...
	if P_Team(self) == TEAM_UNDEAD then
		local speed = self:GetZombieClassTable().Speed * GAMEMODE.ZombieSpeedMultiplier * (speedmultipliers[game.GetMap()] and speedmultipliers[game.GetMap()] or 1)
...
function meta:ShouldBarricadeGhostWith(ent)
	return not ent.IsMapProp and ent:IsBarricadeProp()
end
...
function meta:ShouldNotCollide(ent)
	if E_IsValid(ent) then
		...
		return not ent.IsMapProp and (E_GetDTBool(self, 0) and ent:IsBarricadeProp())
	end
...
function meta:GetMaxZombieHealth()
	if self:GetZombieClassTable().Boss then
