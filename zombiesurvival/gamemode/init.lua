...
AddCSLuaFile("skillweb/registry_prestige.lua")
...
weaponmodelstoweapon["models/props_junk/watermelon01_chunk02a.mdl"] = "weapon_zs_f_watermelon"
weaponmodelstoweapon["models/props_junk/harpoon002a.mdl"] = "weapon_zs_harpoon"
...
local function BossZombieSort(za, zb)
	local ascore = (za.WaveBarricadeDamage * 0.05 + za.WaveHumanDamage) * (za:IsBot() and -1 or 1)
	local bscore = (zb.WaveBarricadeDamage * 0.05 + zb.WaveHumanDamage) * (zb:IsBot() and -1 or 1)
...
function GM:SpawnBossZombie(bossplayers, silent, bossindex, triggerboss)
    if not bossplayers then
        bossplayers = self:CalculateNextBoss()
    end
    --[[if not bossplayers then return end
    ...
    for k, v in pairs(bossplayers) do
        if not triggerboss then
            k.BossDeathNotification = true
            GAMEMODE.StatTracking:IncreaseElementKV(STATTRACK_TYPE_ZOMBIECLASS, GAMEMODE.ZombieClasses[v].Name, "BossSpawn", 1)
        end
        local curclass = k.DeathClass or k:GetZombieClass()
        k:KillSilent()
        k:SetZombieClass(v)
        k:DoHulls(v, TEAM_UNDEAD)
        k.DeathClass = nil
        k:UnSpectateAndSpawn()
        k.DeathClass = curclass
        k.BossHealRemaining = 999999999
        if not silent then
...
function GM:ShouldRestartRound()
	if ZS.TimeLimit == -1 or ZS.RoundLimit == -1 then return true end

	local roundlimit = ZS.RoundLimit

	local timelimit = ZS.TimeLimit
	...
	if NextTick <= time then
		...
		for _, pl in pairs(allplayers) do
			if P_Team(pl) == TEAM_HUMAN and P_Alive(pl) then
				...
				if pl:IsSkillActive(SKILL_ARMOR) and pl:GetMaxArmor() > 0 and time >= pl.NextBodyArmorRegen and pl:Armor() < pl:GetMaxArmor() then
					pl.NextBodyArmorRegen = time + 6
					pl:SetArmor(math.min(pl:GetMaxArmor(), pl:Armor() + (1 * (pl.ArmorGainMul or 1))))
				end
				...
				if pl:IsSkillActive(SKILL_STOWAGE) and self:GetWave() > 0 and time > (pl.NextResupplyUse or 0) then
					...
					net.Start("zs_stowagecaches")
						net.WriteInt(pl.StowageCaches, 16)
					net.Send(pl)
				end
			end
		end
...
function GM:CalculateNextBoss()
    ...
    for _, ent in pairs(team.GetPlayers(TEAM_UNDEAD)) do
        if ent:GetInfo("zs_nobosspick") ~= "1" and not ent:GetZombieClassTable().Boss then
            table.insert(zombies, ent)
        end
    end

	for _, ent in pairs(player.GetBots()) do
    ...
    for k, v in ipairs(zombies) do
        if table.Count(newbosses) >= math.ceil(self:GetWave() / 1.5) then break end
        newbosses[v] = v:GetBossZombieIndex()
    end

    if not newbosses == self.LastCalculatedBoss or CurTime() >= self.LastCalculatedBossTime + 2 then
        self.LastCalculatedBoss = newbosses
        self.LastCalculatedBossTime = CurTime()
        net.Start("zs_nextboss")
        if newbosses and table.Count(newbosses) > 0 then
            net.WriteTable(newbosses)
        else
            net.WriteTable(NULL)
        end
        net.Broadcast()
...
function GM:PlayerRepairedObject(pl, other, health, wep)
	...
	gamemode.Call("PostPlayerRepairedObject", pl, other, health, wep)
end

function GM:PostPlayerRepairedObject(pl, other, health, wep)
end
...
function GM:PlayerInitialSpawnRound(pl)
	...
	if pl:Team() == TEAM_UNDEAD and self.StoredUndeadFrags[uniqueid] then
		...
		pl:SetNWInt("ZSFrags", self.StoredUndeadFrags[uniqueid])
...
function GM:PropBroken(ent, attacker)
	if IsValid(ent) and IsValid(attacker) and not ent._PROPBROKEN and attacker:IsPlayer() and attacker:Team() == TEAM_HUMAN then
		...
		PrintMessage(HUD_PRINTTALK, attacker:LogID().." broke "..ent:GetModel())
	end
end
...
function GM:NestDestroyed(ent, attacker)
	if IsValid(ent) and IsValid(attacker) and attacker:IsPlayer() and attacker:Team() == TEAM_UNDEAD then
		PrintMessage(HUD_PRINTTALK, attacker:LogID().." team killed a nest at "..tostring(ent:GetPos()).." (builder: "..(ent:GetOwner() and ent:GetOwner():IsValid() and ent:GetOwner():IsPlayer() and ent:GetOwner():LogID() or "unknown")..")")
	end
end
...
function GM:PlayerHurt(victim, attacker, healthremaining, damage)
	...
	if victim:Team() == TEAM_HUMAN then
		...
		if healthremaining < victim:GetMaxHealth() * 0.5 and victim:HasTrinket("armorpack") then
			victim:SetArmor(math.min(victim:Armor() + (20 * (victim.ArmorGainMul or 1)), victim:GetMaxArmor() + (20)))
			victim:TakeInventoryItem("trinket_armorpack")

			net.Start("zs_trinketconsumed")
				net.WriteString("Armor Transfusion Pack")
			net.Send(victim)
		end
	else
	...
	elseif key == IN_SPEED then
		if pl:Alive() then
			if pl:Team() == TEAM_HUMAN then
				...
				if not pl:IsCarrying() and pl:KeyPressed(IN_SPEED) and pl:IsSkillActive(SKILL_CARDIOTONIC) and pl:Armor() > 0 then
					pl:SetArmor(pl:Armor() - 1)
...
function GM:PlayerUse(pl, ent)
	...
	if entclass == "prop_door_rotating" then
		if CurTime() < (ent.m_AntiDoorSpam or 0) or pl:Team() == TEAM_UNDEAD then -- Prop doors can be glitched shut by mashing the use button.
			return false
		end
...
local function DelayedChangeToZombie(pl)
	if pl:IsValid() then
		if pl.ChangeTeamFrags then
			...
			pl:SetNWInt("ZSFrags", pl.ChangeTeamFrags)
...
VoiceSetTranslate["models/player/zombie_soldier.mdl"] = VOICESET_ZOMBINE
VoiceSetTranslate["models/player/zombie_classic.mdl"] = VOICESET_ZOMBIE
function GM:PlayerSpawn(pl)
	...
	if pl:Team() == TEAM_UNDEAD then
		...
		if classtab.Material then
			pl:SetMaterial(classtab.Material)
		end

		if classtab.Color then
			pl:SetColor(classtab.Color)
		end
		...
		if not pl.Revived and not classtab.NeverAlive and pl.SpawnedOnSpawnPoint and not pl.DidntSpawnOnSpawnPoint then
			pl:GiveStatus("zombiespawnbuff", self.ObjectiveMap and 1.5 or 3)
		end
		pl.DidntSpawnOnSpawnPoint = nil
		pl.SpawnedOnSpawnPoint = nil

		local overridemodel = classtab.OverrideModel
		if overridemodel then
			timer.Simple(0, function()
				...
				if current and current:IsValid() then
					...
					if classtab.Material then
						current:SetMaterial(classtab.Material)
					end
					current:ResetBones()
		...
		gamemode.Call("PostPlayerSpawnZombie", pl)
	elseif pl:Team() == TEAM_HUMAN then
		pl.PointQueue = 0
		pl.PackedItems = {}
		...
		if self.ZombieEscape then
            ...
            if start then
                for _, v in pairs(start) do
                    local func = self:GetInventoryItemType(v) == INVCAT_TRINKETS and pl.AddInventoryItem or pl.Give
                    func(pl, v)
                end
            end
		...
		gamemode.Call("PostPlayerSpawnHuman", pl)
	end
...
function GM:PostPlayerSpawn(pl)
end

function GM:PostPlayerSpawnHuman(pl)
end

function GM:PostPlayerSpawnZombie(pl)
end
...
function GM:WaveStateChanged(newstate)
	...
	elseif self:GetWave() >= self:GetNumberOfWaves() then -- Last wave is over
		...
		gamemode.Call("OnWaveEnd")
	end

	gamemode.Call("OnWaveStateChanged")
end

function GM:OnWaveStart()
end

function GM:OnWaveEnd()
end
...
function GM:PlayerSwitchFlashlight(pl, newstate)
	...
	if pl:Team() == TEAM_HUMAN and CurTime() >= pl.NextFlashlightSwitch then
		pl.NextFlashlightSwitch = CurTime() + 0.5
		return true
	end
