...
-- Use this table to store the spawned sigil locations
local nodesToMerge = {}
function GM:CreateCustomSigils()
	if not self.CustomSigilNodes then return end

	-- Now we get max sigils, choose a random spot, make sure to remove the sigil's position from the spawnlist and move it onto the stored table to add back later
	for i=1, self.MaxSigils do
        local pick = self.CustomSigilNodes[math.random(#self.CustomSigilNodes)]
        local ent = ents.Create("prop_obj_sigil")
        ent:SetPos(pick)
        ent:Spawn()
        table.remove(self.CustomSigilNodes, table.KeyFromValue(self.CustomSigilNodes, pick))
		table.insert(nodesToMerge, pick)
    end

	-- After the sigils have spawned we then add all of the spawned sigil locations back to the base list and empty the stored table
	table.Add(self.CustomSigilNodes, nodesToMerge)
	table.Empty(nodesToMerge)

	-- Tell the gamemode all the sigils have spawned and we're ready to play
	self:SetUseSigils(self:NumSigils() > 0)
end