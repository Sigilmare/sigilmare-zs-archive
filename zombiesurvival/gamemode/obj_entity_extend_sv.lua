...
function meta:IsNailedToWorld(hierarchy)
	if self:IsNailed() then
		for _, nail in pairs(self.Nails) do
			if nail:GetAttachEntity():IsWorld() or nail:GetAttachEntity().IsMapProp then
				return true
			end
		end
	end
...
function meta:RecalculateNailBonuses()
	...
	local new_max_health = self.OriginalMaxHealth + num_extra_nails * 75
...
timer.Create("CachedInvisibleEntities", 1, 0, function()
	...
	for _, ent in pairs(ents.GetAll()) do
		if isnumber(ent) then continue end
