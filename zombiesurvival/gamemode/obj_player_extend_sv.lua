	...
	if P_Team(self) == TEAM_UNDEAD then
		...
		if attacker:IsValidLivingHuman() and inflictor:IsValid() and inflictor == attacker:GetActiveWeapon() then
			...
			if wep.IsMelee then
				...
				if attacker.MeleeDamageToArmorMul and attacker.MeleeDamageToArmorMul > 0 and attacker:Armor() < attacker:GetMaxArmor() then
					attacker:SetArmor(math.min(attacker:GetMaxArmor(), attacker:Armor() + math.min(damage, self:Health()) * attacker.MeleeDamageToArmorMul * attacker.ArmorGainMul))
				end
...
function meta:GiveStatus(sType, fDie)
	...
	if resistable and self:IsSkillActive(SKILL_HAEMOSTASIS) and self:Armor() >= 10 then
		self:SetBloodArmor(self:GetBloodArmor() - 10)
		return
	end
...
function meta:Resupply(owner, obj)
	...
	if not stowage then
		...
		net.Start("zs_stowagecaches")
			net.WriteInt(self.StowageCaches, 16)
		net.Send(self)
	end
...
function meta:AddPoints(points, floatingscoreobject, fmtype, nomul)
	...
	self:SetNWInt("ZSFrags", self:GetNWInt("ZSFrags") + wholepoints)
	...
	if ZS.XPMul and ZS.XPMul >= 1 then
		xp = xp * self:GetTotalXPMultiplier()
...
function meta:Redeem(silent, noequip)
	...
	if frags < 0 then
		...
		self:SetNWInt("ZSFrags", self:GetNWInt("ZSFrags") * 5)
	else
		self:SetFrags(0)
	end
	self:SetDeaths(0)
...
function meta:GivePointPenalty(amount)
	...
	self:SetNWInt("ZSFrags", self:GetNWInt("ZSFrags") - amount)
...
function meta:ReflectDamage(damage)
	local frags = self:GetNWInt("ZSFrags")
