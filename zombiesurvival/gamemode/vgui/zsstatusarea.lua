...
function statusValueFunctionAlt(statusname)
	return function(self, lp)
		local status = lp:GetStatus(statusname)
		if status and status:IsValid() then
			return math.max(status:GetStartTime() + status:GetEndTime() - CurTime(), 0)
		end
...
local statusdisplays = {
	{
		Color = Color(84, 169, 238),
		Name = "STUNNED!",
		ValFunc = statusValueFunction("stun"),
		Max = 5,
		Icon = Material("zombiesurvival/slow.png")
	},
	{
		Color = Color(148, 90, 105),
		Name = "CONFUSION!",
		ValFunc = statusValueFunctionAlt("confusion"),
		Max = 10,
...
function PANEL:StatusThink(lp)
	...
	elseif self.MemberValue < self.LerpMemberValue then
		self.LerpMemberValue = math.Approach(self.LerpMemberValue, self.MemberValue, FrameTime() * 60)
	end
...
function PANEL:Paint(w, h)
	...
	draw.SimpleTextBlurry((t1 < 999 and t1 or "∞"), (t1 < 999 and "ZSHUDFontSmall" or "ZSHUDFont"), w / 2, y + h / 2 - boxsize/2 + 5, color_white_alpha230, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	draw.SimpleTextBlurry(string.sub(self.MemberName, 1, 4), "ZSHUDFontTiny", w / 2, y + h / 2 - boxsize/2 + 30, self.m_Color, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
	--draw.SimpleText((t1 < 999 and t1 or "∞"), "ZSHUDFontSmall", w / 2, y + h / 2.5 - boxsize/2 + 5, color_white_alpha230, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end
