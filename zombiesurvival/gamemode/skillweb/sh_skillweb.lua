...
function GM:SkillCanUnlock(pl, skillid, skilllist)
	...
	if skill then
		if skill.Prestige and pl:GetZSRemortLevel() < skill.Prestige then
			return false
		end
...
function meta:ApplySkills(override)
	--if GAMEMODE.ZombieEscape or GAMEMODE.ClassicMode or GAMEMODE.ObjectiveMap then return end -- Skills not used on these modes
