...
function meta:SkillsRemort()
	...
	BroadcastLua([[surface.PlaySound("sigilmare/rebirth.ogg")]])
    self:SendLua("util.WhiteOut(54)")
    self:SendLua("util.ScreenShake(MySelf:GetPos(), 2, math.huge, 54, math.huge)")

	self:CenterNotify({CustomTime = 54}, {font = "ZSHUDFontSmall"}, SIGILCOLOR_AETHER, "You have just prestiged to prestige "..rl.."!")
	for _, pl in pairs(player.GetAll()) do
		if pl ~= self then
			pl:CenterNotify({CustomTime = 54}, SIGILCOLOR_AETHER, self, " has just prestiged to prestige "..rl.."!")
		end
	end

	Discord.Backend.API:Send(
        Discord.OOP:New('Message'):SetChannel('Prestiges'):SetEmbed({
            color = 0x80deea,
            title = "Prestige",
            description = self:Name().." has just prestiged to prestige "..rl.."!"
        }):ToAPI()
    )
end
