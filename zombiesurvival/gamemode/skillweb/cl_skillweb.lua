...
net.Receive("zs_skills_nextreset", function(length)
    ...
    if GAMEMODE.SkillWeb and GAMEMODE.SkillWeb:IsValid() then
        ...
        btn:SetText(GAMEMODE.NextSkillReset <= 0 and "Reset" or (days .. " hours left"))
    ...
    for y = 1, h do
        for x = 1, w do
            ...
            if col.r ~= 255 or col.g ~= 255 or col.b ~= 255 then
                ...
                prelines[id] = string.format("SKILL_%d = %d", id, id)
                lines[id] = string.format("GM:AddSkill(SKILL_%d, \"\", \"\",\n%s%d,%s%d,%s{})", id, string.rep("\t", 16), lx, string.rep("\t", 3), ly, string.rep("\t", 3))
            end
        end
    end
...
local particlecolors = {
    ...
    [TREE_P_HEALTH] = SIGILCOLOR_AETHER,
    [TREE_P_SPEED] = SIGILCOLOR_AETHER,
    [TREE_P_SUPPORT] = SIGILCOLOR_AETHER,
    [TREE_P_DEFENCE] = SIGILCOLOR_AETHER,
    [TREE_P_MELEE] = SIGILCOLOR_AETHER,
    [TREE_P_RANGED] = SIGILCOLOR_AETHER,
}
...
local nodecolors = {
    ...
    [TREE_GUNTREE] = {5, 2, 2},
    [TREE_P_HEALTH] = {128/255, 222/255, 234/255},
    [TREE_P_SPEED] = {128/255, 222/255, 234/255},
    [TREE_P_SUPPORT] = {128/255, 222/255, 234/255},
    [TREE_P_DEFENCE] = {128/255, 222/255, 234/255},
    [TREE_P_MELEE] = {128/255, 222/255, 234/255},
    [TREE_P_RANGED] = {128/255, 222/255, 234/255},
}
...
function PANEL:Init()
    ...
    node = ClientsideModel("models/maxofs2d/hover_rings.mdl", RENDER_GROUP_OPAQUE_ENTITY)

    if IsValid(node) then
        node:SetNoDraw(true)
        node:SetPos(Vector(0, 0, 10))
        node:SetModelScale(0.75, 0)
        node.Skill = PRESTIGE_SKILL
        node.SkillID = -1
    ...
    for tree, treenode in pairs(TREE_SKILLS) do
        ...
        if IsValid(node) then
            local rads = (2 * math.pi) * ((tree - 0.5) / #TREE_SKILLS)
            node:SetNoDraw(true)
            node:SetPos(Vector(0, math.sin(rads) * 90, math.cos(rads) * 90 + 12))
            node:SetAngles(Angle(0, 0, -rads * 180 / math.pi))
            node:SetModelScale(0.25, 0)
            node.Skill = treenode
            node.SkillID = -tree - 1
    ...
    reset.DoClick = function(me)
        Derma_Query("Reset all skills and refund SP?", "Warning", "OK", function()
    ...
    spremaining:SetChangeFunction(function() return "Unused skill points: " .. MySelf:GetZSSPRemaining() end, true)
...
function PANEL:Think()
    ...
    if time > self.NextProgressThink then
        ...
        for skill, skillinf in pairs(GAMEMODE.Skills) do
            if not skillinf.Tree then continue end
        ...
        for tree, _ in pairs(TREE_SKILLS) do
            for i, j in pairs(active) do
                ...
                if not skillinf or not skillinf.Tree then continue end
...
function PANEL:Paint(w, h)
    ...
    for i, particle in pairs(particles) do
        ...
        render.DrawQuadEasy(particle[1], to_camera, particle[4], particle[4] * 1.5, particlecolors[self.DesiredTree] or colSmoke, particle[2])
    end
    ...
    for id, node in pairs(skillnodes) do
        if IsValid(node) then
            nodepos = node:GetPos()
            if (nodepos - campost):LengthSqr() > camheightadj then continue end
            skill = node.Skill
            if skill.Disabled then continue end
    ...
    for id, node in pairs(skillnodes) do
        if IsValid(node) then
            ...
            if skillid <= -2 then
                ...
                node:SetModelScale(2)
                node:DrawModel()
                node:SetModelScale(2.1)
            end
            ...
            if self.DesiredZoom < 9500 then
                ...
                local skill_text_y = skillid <= -2 and -130 or 0
                draw_SimpleText(skill.Name, skillid <= -1 and "ZS3D2DFont2Big" or "ZS3D2DFont2Small", 0, skill_text_y, colo, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

                if skillid <= -2 and self.TreeCount[-skillid - 1] then
                    draw_SimpleText((self.Progress[-skillid - 1] or 0) .. "/" .. self.TreeCount[-skillid - 1], "ZS3D2DFont2Big", 0, skill_text_y + 260, colo, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
                end
            end
    ...
    if oldskill ~= hoveredskill then
        ...
        if hoveredskill then
            skill = hoveredskill < -1 and TREE_SKILLS[-hoveredskill - 1] or hoveredskill == -1 and PRESTIGE_SKILL or GAMEMODE.Skills[hoveredskill]
    ...
    if mc == MOUSE_LEFT then
        ...
        if hoveredskill then
            ...
            if hoveredskill == -1 and can_remort then
				Derma_Query(
					"Are you ABSOLUTELY sure you want to prestige?\nYou will lose all skills, experience, skill points, and levels.\nThis cannot be undone!",
					"Warning",
					"OK",
            ...
            elseif hoveredskill == -1 then
				self:DisplayMessage("You need to be level "..GAMEMODE.MaxLevel.." to prestige!", COLOR_RED)
				surface.PlaySound("sigilmare/ui/deny.ogg")
            ...
            elseif MySelf:SkillCanUnlock(hoveredskill) then
                ...
                self:DisplayMessage("You need to unlock other skills to unlock this skill!", COLOR_RED)
                surface.PlaySound("buttons/button8.wav")
...
function PANEL:Think()
    ...
    if new_level ~= self.PlayerLevel then
        if new_level ~= 1 and self.FirstLevelChange then
			GAMEMODE:CenterNotify({font = "ZSHUDFont"}, SIGILCOLOR_RED, "RANK UP!")
			GAMEMODE:CenterNotify({font = "ZSHUDFont"}, SIGILCOLOR_ZSXP, "Level "..new_level)
			if new_level < GAMEMODE.MaxLevel then
				GAMEMODE:CenterNotify({font = "ZSHUDFontSmaller"}, SIGILCOLOR_AETHER, "Rank up "..(GAMEMODE.MaxLevel - new_level).." more "..(new_level == (GAMEMODE.MaxLevel - 1) and "time" or "times").." to prestige")
			else
				GAMEMODE:CenterNotify({font = "ZSHUDFontSmaller"}, SIGILCOLOR_AETHER, "You can now prestige by pressing F3!")
			end
            --util.ScreenShake(MySelf:GetPos(), 10, math.huge, 8, math.huge)
			--MySelf:ScreenFade(SCREENFADE.IN, SIGILCOLOR_ZSXP, 0.9, 0.1)
			surface.PlaySound("sigilmare/levelup.wav")
        else
