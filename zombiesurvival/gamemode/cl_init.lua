...
function GM:ZombieHUD()
	...
	if not self:GetWaveActive() and self:GetWave() ~= 0 and GAMEMODE.NextBossZombies then
		local bosses = GAMEMODE.NextBossZombies
		local count = 1
		for k, v in pairs(bosses) do
			if k and k:IsValid() then
				local x, y = ScrW() / 2, ScrH() * 0.3 + (draw_GetFontHeight("ZSHUDFont") * count)
				if k == MySelf then
					draw_SimpleTextBlur(translate.Format("you_will_be_x_soon", translate.Get(GAMEMODE.ZombieClasses[v].TranslationName)), "ZSHUDFont", x, y, COLOR_SOFTRED, TEXT_ALIGN_CENTER)
				else
					draw_SimpleTextBlur(translate.Format("x_will_be_y_soon", k:Name(), translate.Get(GAMEMODE.ZombieClasses[v].TranslationName)), "ZSHUDFont", x, y, COLOR_GRAY, TEXT_ALIGN_CENTER)
				end
				count = count + 1
			end
		end
...
local fontfamily = "Typenoksidi_Updated"
...
function GM:CreateScalingFonts()
	...
	surface.CreateLegacyFont(fontfamily, screenscale * (20 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFont_20", fontshadow, fontoutline)
	surface.CreateLegacyFont(fontfamily, screenscale * (34 + fontsizeadd), fontweight, fontaa, false, "ZSHUDFont_34", fontshadow, fontoutline)
...
function GM:EvaluateFilmMode()
	...
	if self.HealthModelHUD and self.HealthModelHUD:IsValid() then
        self.HealthModelHUD:SetVisible(visible)
    end
...
function GM:CreateLateVGUI()
	if not self.HealthHUD then
		...
		self.HealthModelHUD = vgui.Create("ZSHealthModelPanel")
	end
...
function GM:PlayerFootstep(pl, vFootPos, iFoot, strSoundName, fVolume)
	if pl:Team() == TEAM_HUMAN then
		if pl:GetModel() == "models/mw2guy/riot/juggernaut.mdl" then
			pl:EmitSound("sigilmare/hellsgamers/roaster/roaster_footstep"..math_random(4)..".ogg")
			return true
		end

		if ZS.Playermodels[pl:SteamID()] and ZS.Playermodels[pl:SteamID()].StepInfo then
			local path = ZS.Playermodels[pl:SteamID()].StepInfo

			pl:EmitSound(path.Sounds[math.random(#path.Sounds)], 75, istable(path.Pitch) and path.Pitch[math.random(#path.Pitch)] or path.Pitch or 100, path.Volume or 1, CHAN_BODY)
			return true
		end
	end
